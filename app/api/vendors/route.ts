  ...
  try {
    ...
    const result = await TimeoutHandler.withTimeout(async () => {
      ...
      const [vendors, total] = await Promise.all([
        Vendor.find(query)
          .select('_id businessName name category location rating isActive isVerified description')
          .sort({ createdAt: -1 })
          .skip((page - 1) * limit)
      ...
      return {
        vendors: vendors.map((v: unknown) => ResponseOptimizer.compressVendor(v as { _id?: string; id?: string; businessName?: string; category?: string; location?: unknown; rating?: unknown; isActive?: boolean; isVerified?: boolean; [key: string]: unknown })),
        pagination: {
          total,
          page,
          limit,
