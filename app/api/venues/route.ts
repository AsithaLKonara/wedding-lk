...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await TimeoutHandler.withTimeout(async () => {
      ...
      const optimizedQuery = QueryOptimizer.optimizeQuery(Venue.find(query) as { lean: () => unknown; select: (fields: Record<string, 1>) => unknown }, ['_id', 'name', 'description', 'location', 'capacity', 'pricing', 'images', 'amenities', 'rating', 'isActive']);
      const queryChain = optimizedQuery as unknown as { sort: (obj: Record<string, 1 | -1>) => { skip: (n: number) => { limit: (n: number) => Promise<unknown[]> } } };
      const [venues, total] = await Promise.all([
        queryChain.sort({ createdAt: -1 }).skip((page - 1) * limit).limit(Math.min(limit, 100)),
      ...
      return {
        venues: venues.map((v: unknown) => ResponseOptimizer.compressVenue(v as { _id?: string; id?: string; name?: string; location?: unknown; pricing?: unknown; images?: string[]; rating?: unknown; isActive?: boolean; [key: string]: unknown })),
        pagination: {
          total,
          page,
          limit,
