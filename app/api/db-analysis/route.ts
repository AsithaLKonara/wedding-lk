...
export async function GET(_request: NextRequest) {
  try {
    ...
    const collectionAnalysis: Record<string, {
      count?: number;
    ...
    const indexAnalysis: Record<string, any> = {};
    for (const collectionName of keyCollections) {
      if (collectionNames.includes(collectionName)) {
        try {
          const collection = db.collection(collectionName);
          const indexes = await collection.indexes();
          indexAnalysis[collectionName] = indexes.map((index: any) => ({
    ...
    const collectionsNeedingIndexes: string[] = [];
    for (const [collectionName, indexes] of Object.entries(indexAnalysis)) {
      if (indexes && !(indexes as any).error && Array.isArray(indexes) && indexes.length < 3) {
