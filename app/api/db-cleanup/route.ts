...
export async function POST(_request: NextRequest) {
  try {
    ...
    const results: {
      ...
      success: boolean;
      errors: Array<{ message: string; operation?: string; error?: string }>;
    ...
    // Remove the old availabilities collection
    try {
      if (!db) {
        throw new Error('Database not connected');
      }
      const availabilitiesExists = await db.listCollections({ name: 'availabilities' }).toArray();
      if (availabilitiesExists.length > 0) {
        const documentCount = await db.collection('availabilities').countDocuments();
    ...
    return NextResponse.json({
      ...
      message: results.success ? 'Database cleanup completed successfully' : 'Some operations failed',
      results
    });
