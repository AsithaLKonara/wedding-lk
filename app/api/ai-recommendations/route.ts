...
interface PackageData {
  ...
  venues?: Array<{ location?: { city?: string } }>;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const personalizedPackages = packages.map((pkg, index) => {
      const pkgData = pkg as unknown as PackageData;
      const matchScore = calculateMatchScore(pkgData, aiContext);
      const pkgWithPrices = pkg as unknown as { originalPrice?: number; price?: number };
      const discount = pkgWithPrices.originalPrice && pkgWithPrices.price && pkgWithPrices.originalPrice > pkgWithPrices.price ? 
        Math.round(((pkgWithPrices.originalPrice - pkgWithPrices.price) / pkgWithPrices.originalPrice) * 100) : 0;

      return {
        ...pkg,
        matchScore,
        discount,
        personalizedFeatures: generatePersonalizedFeatures(pkgData, aiContext),
        whyRecommended: generateWhyRecommended(pkgData, aiContext),
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        venues: venues.map(venue => ({
          ...venue,
          matchScore: calculateVenueMatchScore(venue as unknown as VenueData, aiContext),
          whyRecommended: generateVenueWhyRecommended(venue as unknown as VenueData, aiContext)
        })),
        vendors: vendors.map(vendor => ({
          ...vendor,
          matchScore: calculateVendorMatchScore(vendor as unknown as VendorData, aiContext),
          whyRecommended: generateVendorWhyRecommended(vendor as unknown as VendorData, aiContext)
        })),
        budgetBreakdown,
        timeline,
        culturalInsights,
...
function calculateMatchScore(pkg: PackageData, context: WeddingContext): number {
  let score = 0;
  ...
  const featureMatches = context.preferences.filter((pref) => 
  ...
  const hasLocationMatch = pkg.venues?.some((venue) => 
...
function calculateVenueMatchScore(venue: VenueData, context: WeddingContext): number {
  let score = 0;
  
  // Capacity match (30% weight)
  if (venue.capacity?.max && venue.capacity.max >= context.guestCount) {
    score += 30;
  }
  
  // Budget match (25% weight)
  const budgetRatio = venue.pricing?.basePrice ? (venue.pricing.basePrice / context.budget) : 1;
  if (budgetRatio <= 0.6) {
    score += 25;
...
function calculateVendorMatchScore(vendor: VendorData, context: WeddingContext): number {
  let score = 0;
  ...
  if (vendor.category && relevantCategories.includes(vendor.category)) {
    score += 30;
  }
...
function generatePersonalizedFeatures(pkg: PackageData, context: WeddingContext): string[] {
...
function generateWhyRecommended(pkg: PackageData, context: WeddingContext): string[] {
  const reasons = [];
  ...
  if (pkg.rating?.average && pkg.rating.average >= 4.5) {
    reasons.push('Highly rated by previous couples');
  }
  
  if (pkg.features && pkg.features.length > 6) {
    reasons.push('Comprehensive package with many inclusions');
  }
  
  if (pkg.venues && pkg.venues.length > 0) {
...
function generateVenueWhyRecommended(venue: VenueData, context: WeddingContext): string[] {
  const reasons = [];
  ...
  if (venue.rating?.average && venue.rating.average >= 4.5) {
    reasons.push('Excellent reviews from previous couples');
  }
  
  if (venue.pricing?.basePrice && venue.pricing.basePrice <= context.budget * 0.6) {
...
function generateVendorWhyRecommended(vendor: VendorData, context: WeddingContext): string[] {
  const reasons = [];
  
  if (vendor.rating?.average && vendor.rating.average >= 4.5) {
...
function generateBudgetBreakdown(budget: number, guestCount: number, _location: string) {
  const breakdown = {
...
function generateTimeline(date: string, _location: string) {
