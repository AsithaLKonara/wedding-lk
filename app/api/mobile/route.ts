...
export async function POST(request: NextRequest) {
  return PerformanceMonitor.trackAPIPerformance('mobile-service', async () => {
    try {
      ...
      let result: Record<string, unknown> | null | unknown[] | boolean

      switch (action) {
        case 'user-data':
          result = await MobileAppService.getMobileUserData(data.userId) as Record<string, unknown> | null
          break
        case 'venues':
          result = await MobileAppService.getMobileVenues(data.filters) as unknown[]
          break
        case 'vendors':
          result = await MobileAppService.getMobileVendors(data.filters) as unknown[]
          break
        case 'bookings':
          result = await MobileAppService.getMobileBookings(data.userId) as unknown[]
          break
        case 'push-notification':
          result = await MobileAppService.sendMobilePushNotification(data.userId, data.notification) as boolean
          break
        default:
          return NextResponse.json({ 
