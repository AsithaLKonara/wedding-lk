...
export async function GET(_request: NextRequest) {
  ...
  const results: {
    timestamp: string;
    responseTime: number;
    services: Record<string, { status: string; responseTime: number; message?: string; error?: string }>;
    overallStatus: string;
    summary?: { totalServices: number; healthyServices: number; unhealthyServices: number; healthPercentage: number };
    error?: string;
  } = {
  ...
  try {
    ...
    const healthyServices = Object.values(results.services).filter((service) => service.status === 'healthy').length;
