...
import { Vendor, Venue, Post, Reel, Story } from '@/lib/models';
import { MongoQuery, MongoSortCriteria } from '@/lib/types/mongodb-queries';
import { BaseSearchCriteria } from '@/lib/types/search-criteria';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const searchCriteria: MongoQuery<BaseSearchCriteria> = {};
    ...
    // Build sort criteria
    let sortCriteria: MongoSortCriteria = {};
    switch (sortBy) {
      case 'trending':
    ...
    if (category === 'all' || category === 'vendors') {
      ...
      content.push(...vendors.map(vendor => ({
        ...
        currency: 'LKR',
        tags: vendor.services?.map((s: any) => s.name) || [],
        engagement: {
          likes: 0,
          comments: 0,
          shares: 0,
