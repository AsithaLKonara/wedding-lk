...
import { BookingSearchCriteria } from '@/lib/types/search-criteria';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const query: MongoQuery<BookingSearchCriteria & { isActive: boolean }> = { isActive: true };
...
async function checkAvailability(vendorId: string, venueId: string, date: string, startTime: string, endTime: string) {
  try {
    ...
    const timeSlot = availability.timeSlots.find((slot: any) => 
...
async function calculateDynamicPricing(vendorId: string, services: any[], date: string, startTime: string) {
  try {
    ...
    if (pricingRules) {
      ...
      const timeBasedRule = pricingRules.timeBasedPricing.find((rule: any) => 
        rule.isActive && 
      ...
      const seasonalRule = pricingRules.seasonalPricing.find((rule: any) => 
        rule.isActive && 
