...
async function getGuestList(request: NextRequest) {
  try {
    ...
    if (status && status !== 'all') {
      filteredGuests = filteredGuests.filter((guest: any) => guest.rsvpStatus === status);
    }

    if (side && side !== 'all') {
      filteredGuests = filteredGuests.filter((guest: any) => guest.side === side);
    }
...
async function updateGuestRSVP(request: NextRequest) {
  try {
    ...
    const guest = guestList.guests.find((g: any) => g.id === guestId);
    if (!guest) {
      return NextResponse.json({
        success: false,
...
export const GET = withRateLimit(
  ...
  withAuth(getGuestList)
);

export const POST = withRateLimit(
  rateLimitConfigs.api,
  withAuth(createGuestList)
);

export const PUT = withRateLimit(
  rateLimitConfigs.api,
  withAuth(updateGuestRSVP)
);
