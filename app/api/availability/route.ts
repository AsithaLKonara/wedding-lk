...
import { AvailabilitySearchCriteria } from '@/lib/types/search-criteria';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build query
    const query: MongoQuery<AvailabilitySearchCriteria> = { isActive: true };
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user.role === 'vendor' && availabilityData.vendor !== user.id) {
      return NextResponse.json({
        success: false,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Check permissions
    if (user.role !== 'admin' && 
        (user.role !== 'vendor' || availability.vendor.toString() !== user.id)) {
      return NextResponse.json({
        success: false,
        error: 'Insufficient permissions'
      }, { status: 403 });
