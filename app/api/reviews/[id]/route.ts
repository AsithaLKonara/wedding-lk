...
import { withAuth, requireUser, requireAdmin, AuthRequest } from '@/lib/middleware/auth-middleware';
import { withRateLimit, rateLimitConfigs } from '@/lib/middleware/rate-limit-middleware';

// GET - Get a specific review
async function getReview(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
...
async function deleteReview(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const user = (request as any).user;
    const resolvedParams = await params;
    const { id } = resolvedParams;
...
async function updateVendorRating(vendorId: string) {
  try {
    ...
    const { Vendor } = await import('@/lib/models');
...
export async function GET(
  ...
  return withRateLimit(rateLimitConfigs.public, async (req: NextRequest) => {
    return getReview(req, { params });
  })(request);
}
...
export async function PUT(
  ...
  return withRateLimit(rateLimitConfigs.api, withAuth(async (req: AuthRequest) => {
    return updateReview(req as any, { params });
  }))(request);
}
...
export async function DELETE(
  ...
  return withRateLimit(rateLimitConfigs.api, withAuth(async (req: AuthRequest) => {
    return deleteReview(req as any, { params });
  }))(request);
}