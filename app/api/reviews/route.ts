...
import { Review, Booking, Vendor } from '@/lib/models';
...
import { MongoQuery, MongoSortCriteria } from '@/lib/types/mongodb-queries';
import { ReviewSearchCriteria } from '@/lib/types/search-criteria';
...
async function getReviews(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('🔍 Direct DB query error:', dbError instanceof Error ? dbError.message : String(dbError));
    }
    
    // Simple test query - get any reviews
    const query: MongoQuery<ReviewSearchCriteria> = {};
    ...
    // Build sort criteria
    const sortCriteria: MongoSortCriteria = {};
    switch (sortBy) {
      case 'newest':
    ...
    const avgRatingPipeline: PipelineStage[] = [
      { $match: { ...query, overallRating: { $exists: true } } },
      { $group: { _id: null, average: { $avg: '$overallRating' }, count: { $sum: 1 } } }
    ];
    const avgRating = await Review.aggregate(avgRatingPipeline);
    ...
    const ratingDistribution = await Review.aggregate(ratingDistributionPipeline);
...
async function updateVendorRating(vendorId: string) {
  try {
    ...
    const categoryAverages = Object.keys(categoryTotals).reduce((acc: any, category) => {
