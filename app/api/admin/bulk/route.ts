...
export const POST = withRateLimit(
  ...
  withAdmin(bulkOperations)
);

// Helper functions for bulk operations
import { Model } from 'mongoose';

async function bulkDelete(model: Model<unknown>, ids: string[]) {
...
async function bulkUpdate(model: Model<unknown>, ids: string[], data: Record<string, unknown>) {
  const result = await model.updateMany(
    { _id: { $in: ids } },
    { 
      ...data,
...
async function bulkActivate(model: Model<unknown>, ids: string[]) {
  const result = await model.updateMany(
    { _id: { $in: ids } },
    { 
      isActive: true,
...
async function bulkDeactivate(model: Model<unknown>, ids: string[]) {
  const result = await model.updateMany(
    { _id: { $in: ids } },
    { 
      isActive: false,
...
async function bulkVerify(model: Model<unknown>, ids: string[]) {
  const result = await model.updateMany(
    { _id: { $in: ids } },
    { 
      isVerified: true,
...
async function bulkUnverify(model: Model<unknown>, ids: string[]) {
  const result = await model.updateMany(
    { _id: { $in: ids } },
    { 
      isVerified: false,
