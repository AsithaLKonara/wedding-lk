...
import { EnhancedBooking, VendorPackage } from '@/lib/models';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (packageData.availability.blackoutDates.some((date: Date | string) => 
    ...
    const newBooking = new EnhancedBooking({
      ...
      services: packageData.services.map((service: {
        ...
        price: service.price,
        customizations: (bookingData.customizations && typeof bookingData.customizations === 'object' && service.service._id && service.service._id.toString() in bookingData.customizations) 
          ? (bookingData.customizations as Record<string, unknown[]>)[service.service._id.toString()] || []
          : []
      })),
      schedule: {
        date: bookingDate,
