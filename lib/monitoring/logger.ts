// Winston logger - make it optional to avoid build errors if not installed
let winston: any;
try {
...
// Custom log format
const logFormat = winston ? winston.format.combine(
  winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }),
  winston.format.errors({ stack: true }),
  winston.format.json(),
  winston.format.printf(({ timestamp, level, message, category, userId, requestId, ...meta }: { timestamp?: string; level?: string; message?: string; category?: string; userId?: string; requestId?: string; [key: string]: any }) => {
...
const logger = winston ? winston.createLogger({
  ...
  format: logFormat || winston.format.json(),
  defaultMeta: {
    service: 'weddinglk',
...
if (winston && process.env.NODE_ENV === 'production' && process.env.MONGODB_URI) {
  try {
    ...
    // MongoDB transport not available
  }
}
