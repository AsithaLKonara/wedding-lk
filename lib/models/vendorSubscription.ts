...
VendorSubscriptionSchema.virtual('daysRemaining').get(function() {
  const now = new Date();
  const periodEnd = (this as any).currentPeriodEnd;
...
VendorSubscriptionSchema.virtual('usagePercentage').get(function() {
  ...
  const total = (features?.maxImages || 0) + (features?.maxVideos || 0) + (features?.maxListings || 0);
  const used = (usage?.imagesUsed || 0) + (usage?.videosUsed || 0) + (usage?.listingsCreated || 0);
