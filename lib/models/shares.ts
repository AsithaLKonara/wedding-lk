...
ShareSchema.methods.addDemographicData = function(ageGroup: string, location: any, deviceType: string) {
  // Add age group data
  let ageGroupData = this.analytics.demographics.ageGroups.find((ag: any) => ag.range === ageGroup);
  if (!ageGroupData) {
  ...
  // Add location data
  if (location) {
    let locationData = this.analytics.demographics.locations.find((loc: any) => 
  ...
  // Add device data
  let deviceData = this.analytics.demographics.devices.find((dev: any) => dev.type === deviceType);
  if (!deviceData) {
