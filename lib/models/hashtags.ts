...
HashtagSchema.methods.addRelatedTag = function(relatedTag: string, correlationScore: number, coOccurrenceCount: number) {
  const relatedTags = this.relatedTags as any[];
  const existing = relatedTags.find((rt: any) => rt.tag === relatedTag);
  ...
  relatedTags.sort((a: any, b: any) => b.correlationScore - a.correlationScore);
  
  // Keep only top 20 related tags
  this.relatedTags = relatedTags.slice(0, 20);
...
HashtagSchema.methods.updateAnalytics = function(hour: number, date: Date) {
  ...
  // Update hourly usage
  let hourlyData = analytics.hourlyUsage.find((h: any) => h.hour === hour);
  if (!hourlyData) {
    hourlyData = { hour, count: 0 };
    analytics.hourlyUsage.push(hourlyData);
  }
  hourlyData.count += 1;
  
  // Update daily usage
  const dayKey = date.toISOString().split('T')[0];
  let dailyData = analytics.dailyUsage.find((d: any) => 
    d.date.toISOString().split('T')[0] === dayKey
  );
  if (!dailyData) {
    dailyData = { date: new Date(dayKey), count: 0 };
    analytics.dailyUsage.push(dailyData);
  }
  dailyData.count += 1;
  
  // Update weekly usage
  const weekNumber = (this as any).getWeekNumber(date);
  let weeklyData = analytics.weeklyUsage.find((w: any) => w.week === weekNumber);
  if (!weeklyData) {
  ...
  let monthlyData = analytics.monthlyUsage.find((m: any) => m.month === month);
  if (!monthlyData) {
    monthlyData = { month, count: 0 };
    analytics.monthlyUsage.push(monthlyData);
  }
  monthlyData.count += 1;
...
HashtagSchema.statics.getRelatedHashtags = function(tag: string, limit: number = 10) {
  return this.findOne({ tag }).then((hashtag: any) => {
    if (!hashtag) return [];
...
HashtagSchema.statics.createOrUpdateHashtag = async function(tag: string, contentType: string, userId: string) {
  ...
  if (hashtag) {
    await (hashtag as any).incrementUsage(contentType, userId);
    return hashtag;
  } else {
    ...
    await (newHashtag as any).incrementUsage(contentType, userId);
    return newHashtag;
  }
};
