import mongoose, { Document, Schema, PipelineStage } from 'mongoose';
import { GroupStage } from '@/lib/types/aggregation-pipeline';
...
AnalyticsSchema.virtual('formattedDate').get(function() {
  return (this.date as Date).toISOString().split('T')[0];
});
...
AnalyticsSchema.statics.getAggregatedAnalytics = async function(
  ...
  if (groupBy.length > 0) {
    ...
    const groupStage: GroupStage = { _id: idObj };
