...
ReviewSchema.virtual('averageCategoryRating').get(function() {
  const ratings = this.categoryRatings as { service: number; quality: number; value: number; communication: number; timeliness: number };
...
ReviewSchema.virtual('helpfulScore').get(function() {
  const helpful = (this.helpful as any[]) || [];
  const notHelpful = (this.notHelpful as any[]) || [];
  const total = helpful.length + notHelpful.length;
  return total > 0 ? (helpful.length / total) * 100 : 0;
});
