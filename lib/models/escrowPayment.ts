...
EscrowPaymentSchema.pre('save', function(next) {
  ...
  if (this.isModified('amount') || this.isModified('platformFee')) {
    this.netAmount = (this.amount as number) - (this.platformFee as number);
  }
  
  // Set auto-release date for event-based releases
  if ((this.releaseConditions as any).type === 'event_based' && (this.releaseConditions as any).eventDate) {
    const releaseDate = new Date((this.releaseConditions as any).eventDate);
    releaseDate.setDate(releaseDate.getDate() + ((this.releaseConditions as any).daysAfterEvent || 1));
    (this.releaseConditions as any).autoReleaseDate = releaseDate;
  }
