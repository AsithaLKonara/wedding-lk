...
BookmarkSchema.methods.removeFromCollection = function(collectionId: string) {
  this.collections = this.collections.filter((id: any) => id.toString() !== collectionId);
  return this.save();
};
...
BookmarkSchema.methods.unshareWith = function(userId: string) {
  this.privacy.sharedWith = this.privacy.sharedWith.filter((id: any) => id.toString() !== userId);
