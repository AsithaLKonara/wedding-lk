    ...
    try {
      ...
      for (const admin of admins) {
        await sendEmail({
          ...
          text: `New document verification request from user ${document.userId} for ${document.documentType}. File: ${document.fileName}. Submitted: ${document.uploadDate.toLocaleDateString()}. Please review in admin dashboard.`
        });
      }
    } catch (error) {
