...
export function ChatInterface({ conversationId, onConversationSelect, className = "" }: ChatInterfaceProps) {
      ...
      setConversations(prev => 
        prev.map(conv => 
          conv._id === selectedConversation._id 
            ? { ...conv, lastMessage: message, unreadCount: (conv.unreadCount || 0) + 1 } as unknown as Conversation
            : conv
        )
      )
    }
  ...
  function handleNotification(notification: { type: string; title: string; message: string }) {
