...
const { Service } = await import('../lib/models/service.js');
const { Task } = await import('../lib/models/task.js');
const { Client } = await import('../lib/models/client.js');
const sampleUsers = [
    {
        name: 'John Doe',
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            for (const userData of sampleUsers) {
                ...
                const user = yield User.create(Object.assign(Object.assign({}, userData), { password: hashedPassword }));
                users.push(user);
            }
            console.log(`👥 Created ${users.length} users`);
            // Create venues
            const venues = yield Venue.create(sampleVenues);
            console.log(`🏛️ Created ${venues.length} venues`);
            // Create vendors
            const vendors = yield Vendor.create(sampleVendors);
            console.log(`👨‍💼 Created ${vendors.length} vendors`);
            // Update services with vendor references
            const updatedServices = sampleServices.map((service, index) => (Object.assign(Object.assign({}, service), { vendor: vendors[index % vendors.length]._id })));
            const services = yield Service.create(updatedServices);
            console.log(`🛠️ Created ${services.length} services`);
            // Create reviews
            const reviews = [];
            for (let i = 0; i < sampleReviews.length; i++) {
                const reviewData = Object.assign(Object.assign({}, sampleReviews[i]), { user: users[i % users.length]._id, venue: venues[i % venues.length]._id, vendor: vendors[i % vendors.length]._id });
                const review = yield Review.create(reviewData);
                reviews.push(review);
            }
            console.log(`⭐ Created ${reviews.length} reviews`);
            // Create tasks
            const updatedTasks = sampleTasks.map((task, index) => (Object.assign(Object.assign({}, task), { assignedTo: users[index % users.length]._id })));
            const tasks = yield Task.create(updatedTasks);
            console.log(`📋 Created ${tasks.length} tasks`);
            // Create clients
            const updatedClients = sampleClients.map((client, index) => {
                var _a;
                return (Object.assign(Object.assign({}, client), { planner: ((_a = users.find(u => u.role === 'admin')) === null || _a === void 0 ? void 0 : _a._id) || users[0]._id }));
            });
            const clients = yield Client.create(updatedClients);
            console.log(`👫 Created ${clients.length} clients`);
            // Create sample bookings
            const bookings = yield Booking.create([
                {
                    user: users[0]._id,
                    venue: venues[0]._id,
            ...
            for (const venue of venues) {
                const venueReviews = reviews.filter(r => { var _a; return ((_a = r.venue) === null || _a === void 0 ? void 0 : _a.toString()) === venue._id.toString(); });
                if (venueReviews.length > 0) {
                    const avgRating = venueReviews.reduce((sum, r) => sum + r.rating, 0) / venueReviews.length;
                    yield Venue.findByIdAndUpdate(venue._id, { rating: avgRating });
                }
            }
            for (const vendor of vendors) {
                const vendorReviews = reviews.filter(r => { var _a; return ((_a = r.vendor) === null || _a === void 0 ? void 0 : _a.toString()) === vendor._id.toString(); });
                if (vendorReviews.length > 0) {
                    const avgRating = vendorReviews.reduce((sum, r) => sum + r.rating, 0) / vendorReviews.length;
                    yield Vendor.findByIdAndUpdate(vendor._id, { rating: avgRating });
                }
            }
